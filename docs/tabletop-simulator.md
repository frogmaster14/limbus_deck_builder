...
```text
LTTS1|F:1-I-2|B:5-I-3|M:1-B-1*2,1-C-2-1,5-C-3-2|E:1-E-2|U:5-U-3-1
```
...
- `F`: 전방 인격 코드네임.
- `B`: 후방 인격 코드네임.
- `M`: 20장 메인 덱. 같은 카드는 `코드*수량`으로 압축한다.
- `E`: 선택한 EGO. 없으면 생략한다.
- `U`: 선택한 강화카드. 없으면 생략한다.
...
같은 카드를 여러 장 테스트하려면 `*수량`을 붙인다.

```lua
testCloneLtCardsAfterLoad("K-1*2,1-E-1")
```
...
`DECK CODE` 입력칸에는 덱 코드를 넣고 `IMPORT`를 누른다. 카드 한 장만 꺼낼 때는 가방 설명에 카드 코드네임 하나를 적고 `CARD`를 누른다. 가방 속 원본은 유지되고 입력한 카드 한 장만 가방 옆에 복제된다.
